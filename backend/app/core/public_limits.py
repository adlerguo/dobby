...
async def record_public_key_usage(
    redis: Redis, auth: AppApiKeyContext, usage: dict[str, Any]
) -> None:
    limits = resolve_limits(auth.config)
    ...
    if int(count) > limit:
        ...
        raise HTTPException(
            ...
            detail={
                "error": {
                    "code": "rate_limited",
                    "message": "Public app API key rate limit exceeded.",
                }
            },
...
async def enforce_daily_usage_available(
    redis: Redis, key_id: Any, limits: PublicKeyLimits
) -> None:
    ...
    if limits.daily_cost_quota > 0 and cost_micros >= int(
        limits.daily_cost_quota * 1_000_000
    ):
...
def quota_exceeded(code: str) -> HTTPException:
    return HTTPException(
        ...
        detail={
            "error": {"code": code, "message": "Public app API key quota exceeded."}
        },
    )
...
def resolve_limits(config: dict[str, Any]) -> PublicKeyLimits:
    ...
    return PublicKeyLimits(
        rate_limit_per_minute=coerce_int(
            limits.get("rate_limit_per_minute"),
            settings.public_app_rate_limit_per_minute,
        ),
        daily_request_quota=coerce_int(
            limits.get("daily_request_quota"), settings.public_app_daily_request_quota
        ),
        daily_token_quota=coerce_int(
            limits.get("daily_token_quota"), settings.public_app_daily_token_quota
        ),
        daily_cost_quota=coerce_float(
            limits.get("daily_cost_quota"), settings.public_app_daily_cost_quota
        ),
    )
