...
async def create_agent(db: AsyncSession, *, tenant_id: UUID, user_id: UUID, payload: AgentCreate) -> AgentOut:
    template = await get_template(db, payload.template_id)
    if payload.template_id is not None and template is None:
        raise ValueError("agent_template_not_found")
    if template is not None and template.type != payload.type:
        raise ValueError("agent_template_type_mismatch")

    config = dict(template.default_config if template is not None else {})
    config.update(payload.config)
    persona = payload.persona or config.get("persona")

    await validate_bindings(db, tenant_id=tenant_id, kb_ids=payload.kb_ids, tool_ids=payload.tool_ids)
    ...
    try:
        ...
        await db.rollback()
        raise ValueError(detail_from_integrity_error(exc, "agent_create_conflict")) from exc
    await db.refresh(agent)
    return await agent_out(db, repo, agent)


async def list_agents(db: AsyncSession, *, tenant_id: UUID, status: str | None = None) -> list[AgentOut]:
    repo = AgentRepository(db, tenant_id)
    agents = await repo.list_by_status(status)
    return [await agent_out(db, repo, agent) for agent in agents]


async def get_agent(db: AsyncSession, *, tenant_id: UUID, agent_id: UUID) -> AgentOut | None:
    repo = AgentRepository(db, tenant_id)
...
async def update_agent(db: AsyncSession, *, tenant_id: UUID, agent_id: UUID, payload: AgentUpdate) -> AgentOut | None:
    repo = AgentRepository(db, tenant_id)
    agent = await repo.get_by_id(agent_id)
    if agent is None:
        return None
    ...
    if payload.kb_ids is not None:
        await validate_bindings(db, tenant_id=tenant_id, kb_ids=payload.kb_ids, tool_ids=[])
        await repo.replace_kbs(agent.id, payload.kb_ids)
    if payload.tool_ids is not None:
        await validate_bindings(db, tenant_id=tenant_id, kb_ids=[], tool_ids=payload.tool_ids)
        await repo.replace_tools(agent.id, payload.tool_ids)
    ...
    try:
        ...
        await db.rollback()
        raise ValueError(detail_from_integrity_error(exc, "agent_update_conflict")) from exc
    await db.refresh(agent)
    return await agent_out(db, repo, agent)


async def publish_agent(db: AsyncSession, *, tenant_id: UUID, agent_id: UUID) -> AgentOut | None:
    repo = AgentRepository(db, tenant_id)
    agent = await repo.get_by_id(agent_id)
    if agent is None:
        return None
...
async def get_template(db: AsyncSession, template_id: UUID | None) -> AgentTemplate | None:
...
async def validate_bindings(db: AsyncSession, *, tenant_id: UUID, kb_ids: list[UUID], tool_ids: list[UUID]) -> None:
    for kb_id in kb_ids:
        result = await db.execute(
            select(KnowledgeBase.id).where(KnowledgeBase.id == kb_id, KnowledgeBase.tenant_id == tenant_id)
        )
        if result.scalar_one_or_none() is None:
            raise ValueError(f"kb_not_found:{kb_id}")

    for tool_id in tool_ids:
        result = await db.execute(select(Tool.id).where(Tool.id == tool_id, Tool.tenant_id == tenant_id))
        if result.scalar_one_or_none() is None:
            raise ValueError(f"tool_not_found:{tool_id}")
