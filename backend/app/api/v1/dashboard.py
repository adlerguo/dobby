...
async def get_dashboard_overview(
    ...
    return await build_dashboard_overview(
        db, tenant_id=auth.tenant_id, view=view, period_days=period_days
    )


@router.get(
    "/dashboard/technical",
    response_model=DashboardTechnicalOut,
    summary="Get technical dashboard",
)
async def get_technical_dashboard(
    ...
    status_filter: str | None = Query(
        default=None, alias="status", pattern="^(ok|failed)$"
    ),
    span_type: str | None = Query(default=None),
    auth: AuthContext = Depends(require_perm("dashboard:view")),
    db: AsyncSession = Depends(get_db),
) -> DashboardTechnicalOut:
    return await build_technical_dashboard(
        db,
...
@router.get(
    "/dashboard/executive",
    response_model=DashboardExecutiveOut,
...
async def get_executive_dashboard(
    ...
    return await build_executive_dashboard(
        db, tenant_id=auth.tenant_id, period_days=period_days
    )
