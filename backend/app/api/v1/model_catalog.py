...
from app.services.model_catalog_service import get_model_catalog_item, list_model_catalog
...
@router.get("/{catalog_id}", response_model=ModelCatalogOut, summary="Get model catalog item")
async def get_model_catalog_item_api(
    ...
    if item is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="model_catalog_not_found")
    return item
